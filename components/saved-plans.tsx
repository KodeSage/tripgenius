...
const SavedPlans: React.FC<SavedPlansProps> = ({ trips, formatBudgetLabel, formatTravelModeLabel, deleteTrip }) => {
    ...
    return (
      <Card className="w-full max-w-4xl mx-auto">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-xl font-semibold text-center mb-2">No trips found</h3>
          <p className="text-muted-foreground text-center mb-6">You haven't created any trip plans yet.</p>
          <Button variant="default">Go to "Create New Plan"</Button>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {trips.map((trip) => {
        ...
        return (
          <Card key={trip._id} className="overflow-hidden border-none shadow-lg">
            <CardHeader
              onClick={() => toggleExpand(trip._id)}
              className={`cursor-pointer transition-colors ${isExpanded ? "bg-primary/5" : "hover:bg-primary/5"}`}
            >
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    <MapPin className="h-4 w-4 text-primary" />
                    <CardTitle className="text-xl text-primary">{plan?.location || trip.destination}</CardTitle>
                  </div>
                        ...
                        >
                          <div className="aspect-video w-full overflow-hidden">
                            <img
                              ...
                              onError={(e) => {
                                ;(e.target as HTMLImageElement).src = "https://placehold.co/600x400?text=Hotel+Image"
                              }}
                            />
                            <div className="absolute top-2 right-2">
                              <Badge className="bg-primary text-primary-foreground">★ {hotel.rating}</Badge>
                            </div>
                          </div>
                          <div className="p-4">
                            <h5 className="font-medium line-clamp-1">{hotel.hotelName}</h5>
                            <p className="text-sm text-muted-foreground line-clamp-1 mb-2">{hotel.hotelAddress}</p>
                            <div className="flex items-center justify-between">
                              <p className="text-sm font-medium text-primary">{hotel.price}</p>
                        ...
                        <TabsContent key={day} value={day} className="mt-0">
                          <div className="mb-4">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                              <h5 className="font-semibold text-lg">{plan.itinerary[day].theme}</h5>
                            ...
                            <div className="space-y-4">
                              {plan.itinerary[day].places.map((place, index) => (
                                <Link
                                  ...
                                  <div className="flex flex-col md:flex-row">
                                    ...
                                    <div className="md:w-2/3 p-4">
                                      <div className="flex items-start justify-between">
                                        <h6 className="font-semibold text-lg mb-2">{place.placeName}</h6>
                                        <ExternalLink className="h-4 w-4 text-muted-foreground ml-2 mt-1 flex-shrink-0" />
                                      </div>
                                      <p className="text-muted-foreground mb-3 line-clamp-3">{place.placeDetails}</p>
                                      <div className="flex flex-wrap gap-2 mt-2">
                                        <Badge variant="outline" className="flex items-center gap-1">
                                          <DollarSign className="h-3 w-3" />
                                          {place.ticketPricing}
                                        </Badge>
                                        {place.travelTime !== "N/A (Start Point)" && (
                                          <Badge variant="outline" className="flex items-center gap-1">
                                            <Clock className="h-3 w-3" />
                                            {place.travelTime}
                                          </Badge>
